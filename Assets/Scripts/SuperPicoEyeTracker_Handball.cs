...
public class SuperPicoEyeTracker_Handball : MonoBehaviour
{
    ...
    public void Init()
    {
        ...
        writer.WriteLine("Timestamp;" +
            "leftOpenness;" +
            "rightOpenness;headPos.x;headPos.y;headPos.z;headVec.x;headVec.y;headVec.z;" +
            "centerPos.x;centerPos.y;centerPos.z;centerVec.x;centerVec.y;centerVec.z");
        isOn = true;
    }
    ...
    private void FixedUpdate()
    {
        ...
        try
        {
            ...
            if (writer != null)
            {
                var head_pos = head.transform.position;
                var head_rot= head.transform.rotation;
                var head_vec = Quaternion.Inverse(head_rot) * new Quaternion(0, 0, 1, 0) * head_rot;
                var eye_vec = Quaternion.Inverse(centerRot) * head_vec * centerRot;
                writer.WriteLine($"{DateTime.Now.ToString("HH:mm:ss.ffffff")};" +
                    $"{leftOpenness};" +
                    $"{rightOpenness};{head_pos.x};{head_pos.y};{head_pos.z};{head_vec.x};{head_vec.y};{head_vec.z};" +
                    $"{centerPos.x};{centerPos.y};{centerPos.z};{eye_vec.x};{eye_vec.y};{eye_vec.z}");
            }
